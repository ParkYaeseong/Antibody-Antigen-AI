...
# --- VAE 및 학습 관련 하이퍼파라미터 ---
SEQ_COLUMN = 'vh_sequence' # 또는 'vl_sequence'
...
def train_vae_model(model, train_loader, val_loader, optimizer, num_epochs, device, beta, pad_idx):
    ...
    model_save_path = os.path.join(model_save_dir, 'vh_vae_best.pth') # 모델 파일명 변경
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(os.path.join(model_save_dir, 'vh_vae_best.pth')))
    model.eval()
    with torch.no_grad():
